...
export class BonusToRegularSpinsWithFreeSpin<
...
> extends ResumeRule<TResponse> {
  ...
  ruleCondition(resp: ResponseHolder<TResponse>): boolean {
    return (
      ...
      !!resp.curResponse.freeSpinsInfo?.currentFreeSpinsGroup &&
      resp.curResponse.freeSpinsInfo.currentFreeSpinsGroup.count === 0
    );
  }
}
