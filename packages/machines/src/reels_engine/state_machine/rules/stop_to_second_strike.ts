...
export class StopToSecondStrike<TResponse extends ISpinResponse> extends BaseGameRule<TResponse> {
  ...
  ruleCondition(resp: ResponseHolder<TResponse>): boolean {
    const winLines = resp.curResponse.winLines;
    return winLines && winLines.length > 0;
  }
}
