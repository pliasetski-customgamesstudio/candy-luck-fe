...
export class InitToIdle<TResponse extends ISpinResponse> extends BaseGameRule<TResponse> {
  ...
  ruleCondition(resp: ResponseHolder<TResponse>): boolean {
    return (
      !resp.curResponse.isBonus && !resp.curResponse.isScatter && !resp.curResponse.freeSpinsInfo
    );
  }
}
