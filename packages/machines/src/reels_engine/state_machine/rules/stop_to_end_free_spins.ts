...
export class StopToEndFreeSpins<TResponse extends ISpinResponse> extends BaseGameRule<TResponse> {
  ...
  ruleCondition(resp: ResponseHolder<TResponse>): boolean {
    return (
      ...
      (GameRulesUtils.isFreeSpinsFinishing(resp) ||
        (resp.preResponse && GameRulesUtils.isFreeSpinsFinishEventSkipped(resp)))
    );
  }
}
