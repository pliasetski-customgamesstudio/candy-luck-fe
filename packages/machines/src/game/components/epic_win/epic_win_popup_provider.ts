...
import { EpicWinConfiguration } from './epic_win_configuration';
import { EpicWinStep, EpicWinStepName } from './epic_win_step';
import { SceneCommon } from '@cgs/common';
import { EpicWinPopupController } from '../../common/slot/controllers/epic_win_popup_controller';

export class EpicWinPopupProvider
  ...
  implements IDisposable
{
  ...
  get startAfterShortWinLines(): boolean {
    return this._startAfterShortWinLines;
  }
  ...
  protected createController(): void {
    const view = new EpicWinPopupView(
      this._container,
      this.rootScene,
      this.popupScene,
    ...
    this.popupController = new EpicWinPopupController(
      this._container,
      view,
      this.newLobbyEpicWinConfiguration,
      true,
  ...
  protected subscribeEvents(): void {
    if (this.popupController && this.popupController instanceof EpicWinPopupController) {
      this._epicWinSub = (this.popupController as EpicWinPopupController).epicWinClosed.listen(
        (_e) => {
