...
import { ExternalCanvasSceneObject } from './ExternalCanvasSceneObject';

export interface ILiveCanvasOptions {
  linePoints: Vector2[];
  lineWidth: number;
  lineColor: string;
...
export class PolyLineSceneObject extends SceneObject {
  private canvas: HTMLCanvasElement;
  protected context: CanvasRenderingContext2D;
  protected externalSceneObject: ExternalCanvasSceneObject;
  ...
  constructor(options: ILiveCanvasOptions, renderDevice: RenderDevice) {
    ...
    const padding = Math.max(options.lineWidth * 2, (options.glowSize || 0) * 4); // Add padding for line width and glow
    ...
    this.externalSceneObject = new ExternalCanvasSceneObject(canvas, renderDevice);
    this.externalSceneObject.initialize();
    this.externalSceneObject.position = this.externalSceneObject.position.subtract(
      new Vector2(padding * 0.5 + options.lineWidth * 0.5, padding * 0.5 + options.lineWidth * 0.5)
    );
    ...
    const adjustedPoints = options.linePoints.map(
      (point) => new Vector2(point.x - minX + padding / 2, point.y - minY + padding / 2)
    );
    ...
    this.drawLine(adjustedPoints, options);
    this.externalSceneObject.updateTexture();
  }
